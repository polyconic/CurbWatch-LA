...
diff.putdata([0 if px[:3] == bg else 255 for px in flat.getdata()])
art = flat.crop(diff.getbbox())

# already the right shape? use it untouched
if abs(src.width / src.height - W / H) < 0.02 and src.width >= W:
    flat.resize((W, H), Image.NEAREST).save(ROOT / 'og-image.png', optimize=True)
    print(f'og-image.png {W}x{H} — artwork used as-is'); raise SystemExit
